...
Token *createToken(int capacity) {
  Token *token = gcmall(sizeof(Token));
  ...
  token->data = gccalloc(capacity, sizeof(DataToken));
  token->capacity = capacity;
  token->length = 0;
...
int createTokenId(State *state, int start, int end) {
  ...
  char *id = getTokenId(buf->value, start, _end);

  start = _end + 1;
  char *safetyType = substring(buf->value, start, end);

  DataToken data = {.line = ed->lineNumber,
                    .row = ed->cursorCol,
                    .safetyType = NULL,
                    .type = IDENTIFIER,
                    .value = strdup(id)};
