...
Position getSymbolIndex(const char *input, int start, int end) {
  ...
  for (size_t i = 0; i < SYMBOL_LIST_SIZE; i++) {
    if (symbolList[i].token == input[start]) {
      char open = input[start];
      char close = symbolList[i + 1].token;
 ...
 */
int handleQuotes(char c, int *in_quotes, int *quote_char) {
 ...
 */
int handleBraces(char c, int brace_level, State *state) {
  Input *input = state->input;

  if (islblock(c)) {
    if (input->length > 0 && !isspace(input->content[input->length - 1])) {
      input->content[input->length++] = ' ';
    }
    return 1;
  } else if (isrblock(c)) {
    if (brace_level == 0 && input->length > 0) {
 ...
 */
int handleWhitespace(const char *next_char, int in_quotes, State *state) {
  if (in_quotes) {
    return 0;
  }
 ...
 */
int handleRegularChar(char c, State *state) {

  Input *input = state->input;

  input->content[input->length++] = c;
  return 1;
}
 ...
 */
int shouldAddNewline(int i, int length, char **history, State *state) {

  Input *input = state->input;

  return (i < length - 1 && history[i + 1] && input->length > 0 &&
          !isspace(input->content[input->length - 1]));
}
