...
static bool formatType(Node *node, int id, char *buffer, size_t capacity) {
  ...
  if (type->type != NODE_ARRAY_TYPE)
    return false;

  char element[256];
  if (!formatType(node, type->arrayType.elementType, element, sizeof(element)))
    return false;
...
static bool matchesNode(Node *node, int typeId, RuntimeValue value,
                        Error *error) {
  if (!node || typeId < 0 || typeId >= node->length)
    return true;

  AstNode *type = &node->ast[typeId];
  if (type->type != NODE_ARRAY_TYPE)
    return matchesScalar(annotationName(node, typeId), value);

  if (value.type != VALUE_ARRAY) {
    if (error)
      addRuntimeError(error, ERR_TYPE_MISMATCH, "array", valueTypeName(value.type));
    return false;
  }

  for (int i = 0; i < value.as.array.length; i++) {
    if (!matchesNode(node, type->arrayType.elementType,
                     value.as.array.items[i], error)) {
...
static bool matchesTypeName(const char *type, RuntimeValue value,
                            Error *error) {
  if (!type)
    return true;
...
bool validateTypeName(const char *type, RuntimeValue value, Error *error) {
  if (matchesTypeName(type, value, error))
    return true;
