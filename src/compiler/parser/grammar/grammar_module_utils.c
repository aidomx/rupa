...
int grammarModuleDetectFromRupaRoot(Token *t, int from_pos, int limit) {
  ...
  if ((t->data[from_pos].type != IDENTIFIER && t->data[from_pos].type != KEYWORD) ||
    ...
    return -1;
  if ((t->data[from_pos + 1].type != IDENTIFIER && t->data[from_pos + 1].type != LITERAL_ID) ||
      strcmp(t->data[from_pos + 1].value, "rupa"))
    return -1;
  /* Must NOT be followed by a dot — `from rupa.X` is different */
  if ((from_pos + 2) < limit && t->data[from_pos + 2].type == DOT) return -1;
...
int grammarModuleDetectFromRupa(Token *t, int from_pos, int limit) {
  ...
  if ((t->data[from_pos + 1].type != IDENTIFIER && t->data[from_pos + 1].type != LITERAL_ID) ||
      strcmp(t->data[from_pos + 1].value, "rupa"))
    return -1;
  if (t->data[from_pos + 2].type != DOT) return -1;
  int mod = from_pos + 3;
  if (mod >= limit) return -1;
  if (t->data[mod].type != IDENTIFIER && t->data[mod].type != LITERAL_ID) return -1;
  return mod;
}
...
int grammarModuleDetectFrom(Token *t, int from_pos, int limit, int *end) {
  ...
  while (cur < limit) {
    if ((t->data[cur].type == KEYWORD || t->data[cur].type == IDENTIFIER) &&
      ...
      break;
    if (t->data[cur].type == COMMA) break;
...
char *grammarModuleBuildPath(Token *t, int start, int end) {
  ...
  for (int i = start; i <= end; i++) {
    if (t->data[i].type == DOT) {
      if (i + 1 <= end && (t->data[i + 1].type == SLASH || t->data[i + 1].type == DOT)) total++;
  ...
  int parts = 0;
  for (int i = start; i <= end; i++) {
    if (t->data[i].type == IDENTIFIER || t->data[i].type == LITERAL_ID) parts++;
  }
  ...
  for (int i = start; i <= end; i++) {
    if (t->data[i].type == DOT) {
      ...
      if (!first && (i == start || t->data[i - 1].type != SLASH)) strcat(buf, ".");
